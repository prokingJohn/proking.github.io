...
class RsMambaBlock(nn.Module):
    """
    基于官方 RsMamba 算法的块结构
    核心思路：
    - 状态空间模型 (SSM) 引入合理数逼近 (Rational Function Approximation)。
    - 使用深度的并行卷积形式近似长距离依赖，同时引入 skip connection。
    """
    def __init__(self, dim, d_ffn=128, ssm_kernel=7):
        super().__init__()
        ...
        # RsMamba 核心部分 —— 近似状态空间卷积
        # 使用 depthwise conv 模拟 SSM 卷积核 (官方实现是 CUDA kernel，我们这里用卷积近似）
        self.ssm_conv = nn.Conv1d(
            dim, dim, kernel_size=ssm_kernel,
            padding=ssm_kernel // 2, groups=dim, bias=False
        )

        # 合理数近似 (rational function) 门控
        self.rational_a = nn.Parameter(torch.randn(dim))
        self.rational_b = nn.Parameter(torch.randn(dim))
    ...
    def forward(self, x):
        ...
        # 前馈
        y = self.ffn(x_flat)  # (B,HW,C)

        # 转换为 1D 序列做 SSM 卷积
        y = y.transpose(1, 2)  # (B,C,HW)
        y = self.ssm_conv(y)  # (B,C,HW)

        # Rational 函数门控（近似状态转移）
        # g = (a*y + b) / (|y| + 1) 避免梯度爆炸
        g = (self.rational_a.unsqueeze(0).unsqueeze(-1) * y +
             self.rational_b.unsqueeze(0).unsqueeze(-1)) / (y.abs() + 1.0)

        y = g.transpose(1, 2)  # (B,HW,C)
...
def train_model():
    ...
    best = -1
    log = open('umamba.txt','a',encoding='utf-8')
